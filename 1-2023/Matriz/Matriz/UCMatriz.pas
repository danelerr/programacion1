...
Type
    ...
    Matriz = Class
      ...
      Public
            ...
            procedure pivotear(i, f: word; var p: word);
            procedure qsort(a, b: word);
            procedure quickSort;
            procedure sumarMatriz(m: Matriz);
...
procedure Matriz.sumarMatriz(m: Matriz);
var i, j: Integer;
begin
  if (NroFilas=m.Filas) and (NroColumnas=m.Columnas) then begin
    for I := 1 to NroFilas do
      for J := 1 to NroColumnas do
        Celdas[i, j] := Celdas[i, j] + m.ObtElement(i,j);
  end;
end;
...
function BinToDec(n: real): real;
var dec, p: real;
begin
  p := 1;
  dec := 0;
  while n>0 do begin
    dec := dec + (trunc(n) mod 10)*p;
    p:=p*2;
    n := trunc(n) div 10;
  end;
  result := dec;
...
procedure Matriz.pivotear(i, f: word; var p: word);
var turno: boolean;
begin
  turno := true;
  repeat
...
procedure Matriz.qsort(a, b: word);
var c, n: word;
begin
  n := b-a+1;
  if n>1 then begin
...
//Dimension = NroFilas*NroColumnas
//elementos[i] = celdas[VectorPosF(i), VectorPosC(i)]
